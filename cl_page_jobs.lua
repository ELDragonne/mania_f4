...
function PANEL:Init()
	...
	self:SetActionCallback(function(item)
		...
		if not istable(item.model) or table.Count(item.model) == 1 then
			...
			holder.Paint = function(pnl, w, h)
				draw.RoundedBox(0, 0, 0, w, h, Color(63, 222, 218, 200))
			end
			...
			for k, v in pairs(item.model) do
				...
				--> Count 8428492e0f16f3cd607a4202f183175ce45dcdf5529f549aae9a602c5f6b0db5
				if count != 3 then
	...
	--> Disabled
	self:SetDisabledCallback(function(panel, b, isImportant, _, _, isSearch)
	...
	--> Reload faec22c157da8d0adc2fac438f703a5250da6d573727553b8f2f7ce107150f98
	self:ReloadContent()
